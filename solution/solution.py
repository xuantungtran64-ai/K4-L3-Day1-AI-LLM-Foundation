...
def run_assistant(
    ...
    while True:
        ...
        costs = estimate_cost(prompt_str, reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    lines.append("-" * 120)
    for res in results:
        p = res["prompt"][:37] + "..." if len(res["prompt"]) > 40 else res["prompt"].ljust(40)
        g4o = res["gpt4o_response"][:37] + "..." if len(res["gpt4o_response"]) > 40 else res["gpt4o_response"].ljust(40)
        mini = res["mini_response"][:37] + "..." if len(res["mini_response"]) > 40 else res["mini_response"].ljust(40)
        g4_lat = f"{res['gpt4o_latency']:.2f}s".ljust(15)
        mini_lat = f"{res['mini_latency']:.2f}s".ljust(15)
        lines.append(f"{p} | {g4o} | {mini} | {g4_lat} | {mini_lat}")
    return "\n".join(lines)
